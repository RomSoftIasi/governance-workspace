...
sed "s/%gov-metadata-name%/$GOV_CONTAINER_NAME/g" docker/k8s/templates/gov-deployment.yaml.template \
| sed "s/%gov-app-label%/$GOV_APP_NAME/g" \
...
sed "s/%gov-metadata-name%/$GOV_CONTAINER_NAME/g" docker/k8s/templates/gov-service.yaml.template | sed "s/%gov-app-label%/$GOV_APP_NAME/g" > docker/k8s/gov-service.yaml
